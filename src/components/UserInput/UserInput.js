...
function UserInput (props) {
    ...
    const resetHandler = (event) =>{
        //reset the user state to initial 
        setUserInput(initialUserInput);

    };

    //one generic state change function
    const inputChangeHandler = (input, value) =>{
        setUserInput((prevInput)=>{
            return{
                ...prevInput,
                [input]: value, //set property so we dynamically can acess a property name
                // so we don't set the property name "input" but instead we set the property 
                //which name is stored in input, like: current-savings,yearly-contribution,...
            };
        });
        ...
        <form onSubmit={submitHandler} className="form">
            ...
            <p>
                ...
                <input
                    ...
                    //setting initial value for user before input
